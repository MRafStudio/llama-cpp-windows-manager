...
## Шлюз (gateway) и порты — 8101 ЕДИНАЯ точка для клиентов

- Менеджер (`LlamaCppWindowsManager.exe`) регистрирует в http.sys URL
  `HTTP://+:8101/` — это **gateway** (`AutoLoadGatewayPort`): фасад для клиентов
  (Hermes aux/vision, MemOS и т.п.) с профилями моделей. Клиенты ходят ТОЛЬКО сюда.
- **llama-server (модель) слушает `Port` из LaunchSettings активной сессии**
  (сейчас 8082) — «движок» за шлюзом, клиентам НЕ доступен: порт живёт, пока
  загружена конкретная модель. Встать на 8101 llama-server физически не может:
  URL уже зарезервирован менеджером в http.sys → в логе llama-server честно
...
- Профиль Default шлюза может указывать на другую модель (эталон 35B): запрос к
  ней переключит загруженную модель (политика «Single active») — это долго
  (выгрузка + загрузка). Для текущей задачи слать ID уже загруженной модели.
- Вспомогательные вызовы Hermes на местной модели (`enable_thinking=false`) —
  через шлюз тоже работают: `chat_template_kwargs` доходят до модели.
